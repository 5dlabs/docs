...
async fn main() -> Result<(), ServerError> {
    ...
    println!("📦 Found {} crates needing update:", crates_to_populate.len());
    for config in &crates_to_populate {
        println!("  - {} ({}) {:?}", config.name, config.version_spec, config.features);
    }
    ...
    let _embedding_model = env::var("EMBEDDING_MODEL")
    ...
    let tasks: Vec<_> = crates_to_populate.into_iter().enumerate().map(|(i, crate_config)| {
        ...
        async move {
            println!("\n📥 [{}/{}] Loading documentation for: {}", i + 1, i + 1, crate_name);
            
            // Create population job
            let job_id = db.create_population_job(config_id).await?;
            db.update_population_job(job_id, "running", None, None).await?;
            ...
            let result = match doc_loader::load_documents_from_docs_rs(
                ...
                Err(e) => {
                    ...
                    db.update_population_job(job_id, "failed", Some(&error_msg), None).await?;
            ...
            println!("✅ [{}/{}] Loaded {} documents for {} in {:.2}s",
                i + 1, i + 1, documents.len(), crate_name, doc_time.as_secs_f64());

            if let Some(ref version) = crate_version {
                println!("📦 [{}/{}] Detected version for {}: {}", i + 1, i + 1, crate_name, version);
            }

            if documents.is_empty() {
                println!("⚠️  No documents found for {crate_name}");
                db.update_population_job(job_id, "completed", None, Some(0)).await?;
                return Ok::<_, ServerError>((crate_name, 0, 0.0));
            }

            // Generate embeddings
            println!("🧠 [{}/{}] Generating embeddings for {}...", i + 1, i + 1, crate_name);
            ...
            println!("✅ [{}/{}] Generated {} embeddings for {} in {:.2}s (${:.6})",
                i + 1, i + 1, embeddings.len(), crate_name, embed_time.as_secs_f64(), estimated_cost);
            ...
            db.update_population_job(job_id, "completed", None, Some(embeddings.len() as i32)).await?;
